  ...
  return (
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center gap-10">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        >
          <Avatar className="w-24 h-24 sm:w-48 sm:h-48 rounded-full overflow-hidden">
            {image ? (
              <AvatarImage
                src={image}
                alt="profile"
              ...
              <div
                className={`uppercase w-24 h-24 sm:w-48 sm:h-48 text-5xl border-[.5px] flex items-center justify-center rounded-full ${getColor(
                  selectedColor
                )}`}
              >
                {firstname
          ...
          <div className="flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center justify-center">
             ...
             <div className="w-full flex gap-5">
                {colors.map((color, index) => <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${selectedColor === index ? "outline outline-white outline-2" : ""} `} key={index} onClick={() => setSelectedColor(index)} ></div>)}
             </div>
          </div>
        </div>
        <div className="w-full">
